...
public abstract class PanelListAdapter {
    ...
    private TextView tv_title_two;//左上角title第二个
    private LinearLayout ll_row;//上方的表头
    private ListView lv_column;//左边的表头第一个
    private ListView lv_column_two;//左边表头第二个
    ...
    private int titleHeight=170;
    private int titleWidth=300;//像素
    private int columnItemHeight;//左边列表的行高取决于内容列表的行高

    private String title = "",titleTwo="";
    ...
    void initAdapter() {
        ...
        lv_column_two.setOnScrollListener(verticalScrollListener);
    }
    ...
    private void reorganizeViewGroup() {
        ...
        titlePaint.setColor(context.getResources().getColor(R.color.color_646464));
        titlePaint.setTextSize(CommonUtils.dp2px(context,15));
        ...
        // 2. title2 (TextView --> PanelListLayout)
        tv_title_two = new TextView(context);
        tv_title_two.setText(titleTwo);
        if (titleBackgroundResource != 0) {
            tv_title_two.setBackgroundResource(titleBackgroundResource);
        }

//        titlePaint.setColor(context.getResources().getColor(R.color.color_646464));
//        titlePaint.setTextSize(CommonUtils.dp2px(context,15));
        tv_title_two.setGravity(Gravity.CENTER);
        tv_title_two.setBackgroundColor(Color.parseColor(titleColor));
        tv_title_two.setId(View.generateViewId());//设置一个随机id，这样可以保证不冲突
        RelativeLayout.LayoutParams lp_tv_title_two = new RelativeLayout.LayoutParams(titleWidth, titleHeight);
        lp_tv_title_two.addRule(RelativeLayout.RIGHT_OF,tv_title.getId());
        lp_tv_title_two.leftMargin=2;
        pl_root.addView(tv_title_two, lp_tv_title_two);
        ...
        lp_mhsv_row.addRule(RelativeLayout.END_OF, tv_title_two.getId());
        lp_mhsv_row.addRule(RelativeLayout.ALIGN_PARENT_TOP);
        lp_mhsv_row.leftMargin=2;
...
//        lv_column.setDivider(context.getResources().getDrawable(R.drawable.column_item_divider));
        ...
        //4. colum2
        lv_column_two = new ListView(context);
        lv_column_two.setDividerHeight(0);
        lv_column_two.setBackgroundColor(Color.parseColor(columnColor));
        lv_column_two.setId(View.generateViewId());
        lv_column_two.setVerticalScrollBarEnabled(false);//去掉滚动条
//        lv_column.setDivider(context.getResources().getDrawable(R.drawable.column_item_divider));
        RelativeLayout.LayoutParams lp_lv_column_two = new RelativeLayout.LayoutParams(titleWidth, ViewGroup.LayoutParams.MATCH_PARENT);
        lp_lv_column_two.addRule(RelativeLayout.BELOW, tv_title.getId());
        lp_lv_column_two.addRule(RelativeLayout.RIGHT_OF,lv_column.getId());
        pl_root.addView(lv_column_two, lp_lv_column_two);
        ...
        lp_srl.addRule(RelativeLayout.RIGHT_OF, lv_column_two.getId());
...
//                ll_contentItem = (LinearLayout) lv_content.getChildAt(lv_content.getFirstVisiblePosition());//获得content的第一个可见item
                ...
                lv_column_two.setSelection(initPosition);
            }
        });
    }
    ...
    private void initColumnTwoLayout() {
        columnItemHeight = ll_contentItem.getHeight();
        lv_column_two.setAdapter(getColumnTwoAdapter());
        if (columnDivider != null) {
            lv_column.setDivider(columnDivider);
        }
    }
     ...
     * @return adapter of column ListView
     */
    private BaseAdapter getColumnTwoAdapter() {
        if (columnTwoAdapter == null) {
            columnTwoAdapter = new ColumnAdapter(context, android.R.layout.simple_list_item_1, getColumnDataList());
        }
        return columnAdapter;
    }
...
//                    lv_column_two.setSelectionFromTop(position,top);
//                } else if (subView != null && (view == lv_column||view==lv_column_two)) {
//                    int top = subView.getTop();
//                    int position = view.getFirstVisiblePosition();
...
//            }
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            if (subView != null && view == lv_content) {
                ...
                lv_column_two.setSelectionFromTop(position,top);
                lv_column.setSelectionFromTop(position,top);
            }else if (subView != null && view == lv_column_two) {
                int top = subView.getTop();
                int position = view.getFirstVisiblePosition();
                lv_content.setSelectionFromTop(position, top);
                lv_column.setSelectionFromTop(position, top);
    ...
    private class ColumnAdapter extends ArrayAdapter {
        ...
        public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
           ...
           // ((TextView) view).setText(columnDataList.get(position));
            ((TextView) view).setText("2015-01-11 03：23");
            if (position%2==0){
                view.setBackgroundColor(context.getResources().getColor(R.color.color_f6f6f6));
            }else{
