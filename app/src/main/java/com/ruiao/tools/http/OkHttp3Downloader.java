...
import com.ruiao.tools.http.log.LoggerInterceptor;
...
public final class OkHttp3Downloader implements Downloader {
    ...
    private static File createDefaultCacheDir(Context context) {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            cache = new File(context.getApplicationContext().getExternalCacheDir(), PICASSO_CACHE);
        } else {
            cache = new File(context.getApplicationContext().getCacheDir(), PICASSO_CACHE);
        }
        if (!cache.exists()) {
    ...
    private static OkHttpClient defaultOkHttpClient(File cacheDir, long maxSize) {
        ...
        return builder.cache(new Cache(cacheDir, maxSize)).build();
    }
